...
module TaskBuilder =
    ...
    type StateMachine<'r>() as this =
        ...
        member __.Await(t:Task< 'r>) : unit =
            ...
            cont <- fun () -> methodBuilder.SetResult t.Result
            methodBuilder.AwaitUnsafeOnCompleted(&awaiter, &this)               
    ...
    type ParentInsensitiveTaskBuilder<'r>() =
        ...
        member inline x.Bind< ^inp 
                ...
                if awt.IsCompleted then
                    ...
                    x.StateMachine.Await(awt, continuation >> x.StateMachine.Result)  
